FROM amd64/ubuntu:disco

RUN apt-get update \
 && apt-get -y dist-upgrade \
 && apt-get -y install -o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confnew -y --force-yes --no-install-suggests --no-install-recommends libpam-systemd- policykit.*- colord- dconf-service-  apt-transport-https automake autotools-dev \
